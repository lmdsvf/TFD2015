...
public class StartServicingClient extends Thread {
	...
	public void run() {
		...
		System.out.println(Integer.parseInt(state.getProperties().getProperty(
				"PClient")));
		System.out.println(Integer.parseInt(state.getProperties().getProperty(
				"PClient"))
				+ state.getReplica_number());
		...
		// for (int i = state.getCommit_number(); i < state.getLog().size();
		// i++) {
		...
		public void run() {
			...
			case REQUEST:
				System.out.println("Request Message received from client: "
						+ msg.getClient_Id());
				System.out.println("msg request number: "
						+ msg.getRequest_Number());
				System.out.println("server clientTable request number: "
						+ state.getClientTable().get(clientId)
								.getRequest_number());
				System.out
						.println(msg.getRequest_Number() == (state
								.getClientTable().get(clientId)
								.getRequest_number() + 1));
				if (msg.getRequest_Number() == (state.getClientTable()
					...
					server.broadcastToServers(prepare,
							state.getConfiguration(), state.getUsingAddress(),
							false);
